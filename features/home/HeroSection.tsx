...
function ToolSvg({ shape, size, color }: { shape: string; size: number; color: string }) {
  const s = size;
  if (shape === "wrench") return (
  ...
  if (shape === "drop") return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none">
      <path
        d="M12 2.25c0 0-7.5 8.438-7.5 13.5a7.5 7.5 0 0 0 15 0C19.5 10.688 12 2.25 12 2.25Z"
        stroke={color} strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"
      />
      <path d="M9 16.5a3 3 0 0 0 3 3" stroke={color} strokeWidth={1.5} strokeLinecap="round" opacity={0.5} />
    </svg>
  );
  if (shape === "gear") return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none">
      <path
      ...
      />
      <path d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" stroke={color} strokeWidth={1.5} />
    </svg>
  );
  if (shape === "pipe") return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none">
      <rect x="3" y="10" width="18" height="4" rx="2" stroke={color} strokeWidth={1.5} />
      <path d="M8 10V6a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v4" stroke={color} strokeWidth={1.5} strokeLinecap="round" />
      <path d="M8 14v4a2 2 0 0 0 2 2h4a2 2 0 0 0 2-2v-4" stroke={color} strokeWidth={1.5} strokeLinecap="round" />
    </svg>
  );
...
export function HeroSection() {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-gradient-to-br from-brand-950 via-brand-900 to-brand-800 dark:from-neutral-950 dark:via-brand-950 dark:to-neutral-900">
      ...
      {/* Floating plumbing tools — right half only so they don't obscure text */}
      {floatingTools.map((tool) => (
        <motion.div
          key={tool.id}
          className={`absolute hidden lg:block animate-${tool.animation}`}
          style={{
            left: tool.x,
            top: tool.y,
          ...
          animate={{ opacity: tool.opacity, scale: 1 }}
          transition={{ delay: 0.5 + tool.delay, duration: 0.8, ease: "easeOut" }}
        >
          <ToolSvg shape={tool.shape} size={tool.size} color={tool.color} />
        </motion.div>
      ))}
